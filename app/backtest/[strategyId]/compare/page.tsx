...
// Update imports to use Sonner instead of the deprecated toast
...
// Add this function before the BacktestComparePage component
const applyParameters = async (strategyId: string, parameters: any) => {
    try {
        // In a real implementation, we would fetch the strategy first
        // and then update only the parameters
        await updateStrategy(Number(strategyId), {
            parameters,
            // These are placeholder values since we don't have the full strategy data
            name: "Updated Strategy",
            description: "Strategy updated with best performing parameters",
...
export default function BacktestComparePage() {
    ...
    // Inside the BacktestComparePage component, add this state
    ...
    // Update this function to use Sonner toast
    const handleApplyParameters = async () => {
        if (!bestRun || !params.strategyId) return

        setIsApplyingParams(true)

        try {
            const bestRunData = comparisonData.find((run) => run.version === bestRun.version)
            if (!bestRunData || !bestRunData.params) {
                throw new Error("Could not find parameters for best run")
            }

            const success = await applyParameters(params.strategyId as string, bestRunData.params)

            if (success) {
                // Use Sonner toast instead of the deprecated toast component
                toast.success("Parameters Applied", {
                    description: `The parameters from Run #${bestRun.version} have been applied to your strategy.`,
                    action: {
                        label: "View Strategy",
            ...
            toast.error("Error", {
                description: "Failed to apply parameters to strategy. Please try again.",
            })
        } finally {
                    ...
                    {bestRun && (
                        <Card className="mb-6 bg-green-50 border-green-200">
                            ...
                            <CardContent>
                                ...
                                {/* Inside the Best Performer card's CardContent, after the Performance Summary section, add: */}
