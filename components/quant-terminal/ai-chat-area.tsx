  ...
  const renderLine = (line: string, idx: number) => {
    ...
    for (const part of parts) {
      if (keywords.includes(part)) {
        tokens.push(
          <span key={keyIdx++} className="text-violet-400">
            {part}
          </span>,
        );
      } else if (
        functions.includes(part) ||
        ...
        tokens.push(
          <span key={keyIdx++} className="text-amber-400">
            {part}
          </span>,
        );
      } else if (/^(BTC|ETH)\/USDT$/.test(part)) {
        tokens.push(
          <span key={keyIdx++} className="text-blue-400">
            {part}
          </span>,
        );
  ...
  return (
    <div className="bg-[#1a1b26] rounded-lg overflow-hidden border border-white/10 min-w-0">
      <div className="flex items-center justify-between px-3 py-2 border-b border-white/10">
        ...
        <span className="font-mono text-[10px] text-blue-400 flex-shrink-0 ml-2">
          {asset} · {timeframe}
        </span>
      </div>
...
function StrategyCard({
  ...
  message: Message;
  onUseStrategy?: (strategy: StrategyData) => void;
}) {
      ...
      <div className="p-4 flex flex-col gap-4 min-w-0">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {strategy.metrics.map((metric, i) => (
            <div
              key={i}
              className="bg-muted/50 border border-border/50 rounded-lg p-2.5 min-w-0"
            >
                ...
                <div className="flex justify-start">
                  <div className="max-w-[80%] min-w-0 bg-card border border-border/50 border-l-[3px] border-l-violet-500 rounded-xl animate-in fade-in slide-in-from-bottom-1 duration-300 shadow-sm">
                    <div className="flex items-center gap-2 px-3 py-2 border-b border-border/50 bg-muted/50">
                      ...
                      <span className="text-xs font-semibold text-violet-500 flex-1">
                        ASTRA AI
                      </span>
...
<p>本月实盘总收益：<span style="color:#10b981">+12.4%</span> (¥5,952)</p>`,
  ...
  if (
    lowerQuery.includes("7天") ||
    lowerQuery.includes("赚钱") ||
    lowerQuery.includes("短期") ||
    lowerQuery.includes("短线")
  ) {
    return {
      type: "strategy",
      content:
        "针对最近7天行情优化的短线策略，抓住短期趋势波段。BTC 近期走势较好，策略信号活跃。",
      strategy: {
        title: "短线趋势 BTC",
        description:
          "针对最近7天行情优化的短线策略，抓住短期趋势波段。BTC 近期走势较好，策略信号活跃。",
