...
export function drawBacktestChart(
  ...
  gridStops.forEach((t) => {
    ...
    ctx.textAlign = 'right'
    const label = t === 0.5 ? '0%' : (t > 0.5 ? '+' : '') + Math.round((mn + (mx - mn) * t) * 0.4) + '%'
  ...
  // Animation dot
  if (options.showAnimation && options.currentIndex !== undefined && options.currentIndex < pts.length - 1) {
    ctx.beginPath()
...
export function drawLiveChart(
  ...
  gridStops.forEach((t) => {
    ...
    ctx.font = '9px JetBrains Mono, monospace'
    ctx.textAlign = 'right'
    const label = t === 0.5 ? '0%' : (t > 0.5 ? '+' : '') + Math.round((mn + (mx - mn) * t) * 0.3) + '%'
    ctx.fillText(label, PAD.l - 4, y + 3)
  })
  ...
  sigs.forEach((s) => {
    ...
    ctx.strokeStyle = s.type === 'buy' ? 'rgba(16, 185, 129, 0.2)' : 'rgba(239, 68, 68, 0.2)'
    ctx.lineWidth = 2
    ctx.stroke()
  })
...
export function generateBacktestData(): { pts: number[]; sigs: Signal[] } {
  ...
  const sigs: Signal[] = []
  for (let i = 2; i < 88; i++) {
    const d = pts[i] - pts[i - 2]
    if (d > 2.2 && (sigs.length === 0 || sigs[sigs.length - 1].type === 'sell')) {
      sigs.push({ i, type: 'buy' })
    }
    if (d < -1.5 && sigs.length > 0 && sigs[sigs.length - 1].type === 'buy') {
...
export function generatePaperData(): { pts: number[]; sigs: Signal[]; ref: number[] } {
  const pts: number[] = []
  let v = 0
  for (let i = 0; i < 120; i++) {
  ...
  for (let i = 4; i < 116; i++) {
    ...
    if (d > 1.5 && (sigs.length === 0 || sigs[sigs.length - 1].type === 'sell')) {
      sigs.push({ i, type: 'buy' })
    }
