...
function getStageLabel(state: StrategyState): { text: string; color: string } {
  const { stages } = state;
  if (stages.live === "running")
    return { text: "● 实盘中", color: "text-emerald-500" };
  if (stages.live === "paused")
    return { text: "⏸ 实盘暂停", color: "text-amber-500" };
  if (stages.live === "stopped")
    return { text: "■ 已终止", color: "text-red-500" };
  if (stages.live === "done")
    return { text: "✓ 已归档", color: "text-muted-foreground" };
  if (stages.paper === "running")
    return { text: "● 模拟中", color: "text-violet-500" };
  if (stages.paper === "paused")
    return { text: "⏸ 模拟暂停", color: "text-amber-500" };
  if (stages.paper === "done")
    return { text: "✓ 模拟完成", color: "text-violet-500" };
  if (stages.bt === "running")
    return { text: "● 回测中", color: "text-blue-500" };
  if (stages.bt === "done")
    return { text: "✓ 回测完成", color: "text-blue-500" };
  if (stages.bt === "ready")
    return { text: "待回测", color: "text-muted-foreground" };
    ...
    if (st === "running" || st === "paused") {
      return (
        <span
          ...
          style={{ backgroundColor: STAGE_COLOR[k] }}
        />
      );
    }
  }
          ...
          {ret.live && (
            <span
              className={cn(
                ...
                ret.live.startsWith("+") ? "text-emerald-500" : "text-red-500",
              )}
            >
              实盘{ret.live}
            </span>
...
function FamilyGroup({
  ...
  const liveRunning = family.members.find(
    (m) => strategyStates[m.id]?.stages.live === "running",
  );
  const paperRunning = family.members.find(
    (m) => strategyStates[m.id]?.stages.paper === "running",
  );
  ...
  const summaryLabel = summaryState
    ? getStageLabel(strategyStates[summaryState.id])
    : null;
    ...
    >
      ...
      <div
        ...
        className={cn(
          ...
          hasActiveMember
            ? "bg-gradient-to-r from-violet-500/8 to-blue-500/5"
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="font-mono text-[9px] text-muted-foreground mt-0.5">
            ...
            {summaryLabel && (
              <span className={cn("ml-1.5", summaryLabel.color)}>
                {summaryLabel.text}
              </span>
            )}
          </div>
      ...
      {!isCollapsed && (
        <div className="px-2 pb-2 pt-1 bg-card space-y-0.5">
          ...
          {[...family.members].reverse().map((member) => {
            ...
            const isLatest =
              member.version ===
              Math.max(...family.members.map((m) => m.version));
            return (
              <MemberRow
                key={member.id}
                strategy={member}
