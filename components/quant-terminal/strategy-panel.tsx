    ...
    >
      <div
        ...
        style={{
          ...
          backgroundColor:
            (isDone || isActive) && !isLocked ? color : "transparent",
          color: isLocked
            ? "currentColor"
            : isDone || isActive
              ? "#fff"
...
function getTabAccess(stages: StrategyStages) {
  ...
  const isFullyArchived =
    stages.live === "done" && stages.paper === "done" && stages.bt === "done";
      ...
      >
        <div
          className={cn(
            ...
            panelCollapsed ? "opacity-0 pointer-events-none" : "opacity-100",
          )}
        >
          {/* Header */}
              ...
              <div className="mx-5 mt-4 px-3 py-2 rounded-lg bg-muted/80 border border-border/60 flex items-center gap-2">
                ...
                {canImprove ? (
                  <button
                    ...
                    className="ml-auto font-mono text-[10px] text-violet-500 hover:underline"
                  >
                    🔧 优化此策略 →
                  </button>
                ) : (
