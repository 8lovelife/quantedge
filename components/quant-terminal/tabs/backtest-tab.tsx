...
export function BacktestTab({ onStartPaper, onStartBacktest, viewOnly, viewOnlyReason, readOnly, onClone }: BacktestTabProps) {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (idx < total - 1) {
        ...
        addLog('回测', `<span class="hi">完成</span> · ${RANGE_LABEL[btRange]} · 胜率 61.3%`)
      }
    }
      ...
      <div className="flex flex-col gap-4 flex-1">
        ...
        <div className="flex items-center gap-2.5 mb-2.5">
          ...
          <div className="flex-1 h-1 bg-muted rounded-sm overflow-hidden">
            <div className="h-full bg-blue-500 rounded-sm transition-all duration-300" style={{ width: `${progress}%` }} />
          </div>
          <span className="font-mono text-[10px] text-blue-500 min-w-[40px] text-right font-medium">{progress}%</span>
        </div>
    ...
    return (
      <div className="flex flex-col gap-4 flex-1">
        ...
        {viewOnly && viewOnlyReason === 'paper' && (
          <div className="px-3 py-2 rounded-lg bg-violet-500/10 border border-violet-500/25 flex items-center gap-2">
            ...
            <span className="font-mono text-[10px] text-violet-500">
              模拟交易进行中 · 回测结果仅供参考 · 想修改策略请先暂停模拟，然后点击「优化此策略」
            </span>
          </div>
        )}
        {viewOnly && viewOnlyReason === 'live' && (
          <div className="px-3 py-2 rounded-lg bg-red-500/10 border border-red-500/25 flex items-center gap-2">
            ...
            <span className="font-mono text-[10px] text-red-500">
              实盘运行中 · 历史回测数据仅供查看 · 需修改策略请先终止实盘，再点击「优化此策略」
            </span>
          </div>
        )}
        {readOnly && (
          <div className="px-3 py-2 rounded-lg bg-muted/80 border border-border/60 flex items-center gap-2">
            ...
            <span className="font-mono text-[10px] text-muted-foreground">已归档 · 仅供查看</span>
          </div>
        )}
        ...
        <div className="grid grid-cols-4 gap-2.5">
          {[
            { label: '总收益', value: '+34.2%', sub: RANGE_LABEL[btRange], color: 'text-emerald-500' },
            { label: '稳定性', value: '1.82', sub: '夏普比率', color: 'text-cyan-500' },
            { label: '最大回撤', value: '-8.4%', sub: '可控范围', color: 'text-red-500' },
            { label: '胜率', value: '61.3%', sub: '57笔', color: 'text-foreground' },
          ].map((s, i) => (
            <div key={i} className="bg-card border border-border/50 rounded-xl p-3 shadow-sm">
              <div className="font-mono text-[9px] text-muted-foreground mb-1.5 tracking-wider font-medium">{s.label}</div>
        ...
        <div>
          <div className="font-mono text-[10px] text-muted-foreground tracking-wider mb-2 font-medium uppercase">
            回测净值曲线 — {strategy_asset_display(state)} · {RANGE_DATES[btRange]}
          </div>
        ...
        <div className="flex gap-2.5">
          {/* "Improve" clone button — always shown unless fully archived */}
          {!readOnly && onClone && (
            <Button
              onClick={onClone}
