    ...
    <div className="flex flex-col gap-4 flex-1">
      ...
      {isPaused && (
        <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/30 border-l-[3px] border-l-amber-500">
          <div className="text-sm font-medium text-amber-500 mb-0.5">&#9208; 策略已暂停</div>
          <div className="text-[11px] text-amber-500/80">策略信号检测已停止，<strong>持仓保持不动</strong>，资金仍在市场中，可随时重启继续运行。</div>
        </div>
      )}
      {isStopped && (
        <div className="p-3 rounded-lg bg-red-500/10 border border-red-500/30 border-l-[3px] border-l-red-500">
          <div className="text-sm font-medium text-red-500 mb-0.5">&#9632; 策略已终止</div>
          <div className="text-[11px] text-red-500/80">所有持仓已强制平仓，资金已退回账户。可重启开启新一轮运行，或归档彻底结束此策略生命周期。</div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-2.5">
        ...
        <div className={`bg-card border rounded-xl p-3 shadow-sm ${isStopped ? 'border-red-500/20' : isPaused ? 'border-amber-500/20' : 'border-border/50'}`}>
          <div className="font-mono text-[9px] text-muted-foreground mb-1.5 tracking-wider font-medium">当前持仓</div>
          <div className={`font-mono text-xl font-semibold ${isStopped ? 'text-red-500' : 'text-foreground'}`}>{holdingLabel}</div>
      ...
      <div className="relative">
        <div className="flex items-center justify-between mb-1.5">
          <span className="font-mono text-[10px] text-muted-foreground tracking-wider font-medium uppercase">
            实盘净值 — {isStopped ? '终止时快照' : isArchived ? '归档快照' : '实时账户'}
          </span>
          <span className={`font-mono text-[10px] font-medium ${isStopped ? 'text-red-500' : isArchived ? 'text-muted-foreground' : isPaused ? 'text-amber-500' : 'text-emerald-500'
            }`}>
            {isStopped ? '&#9632; 已终止' : isArchived ? '&#128193; 已归档' : isPaused ? '&#9208; 暂停中' : '● LIVE'}
          </span>
        </div>
        ...
        {isStopped && (
          <div className="absolute inset-0 top-6 rounded-lg bg-red-500/5 border border-red-500/10 flex items-center justify-center pointer-events-none">
            <span className="font-mono text-[10px] text-red-500/50 tracking-widest uppercase">已平仓 · 数据冻结</span>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-4 gap-2.5">
        <div className="bg-card border border-border/50 rounded-xl p-3 shadow-sm">
          ...
          <div className="text-[10px] text-muted-foreground mt-1">低于止损线</div>
        </div>
        <div className="bg-card border border-border/50 rounded-xl p-3 shadow-sm">
        ...
        <div className="bg-card border border-border/50 rounded-xl p-3 shadow-sm">
          <div className="font-mono text-[9px] text-muted-foreground mb-1.5 tracking-wider font-medium">夏普比率</div>
          <div className="font-mono text-xl font-semibold text-cyan-500">1.74</div>
          <div className="text-[10px] text-muted-foreground mt-1">风险调整后</div>
        </div>
        <div className="bg-card border border-border/50 rounded-xl p-3 shadow-sm">
          ...
          <div
            ...
            dangerouslySetInnerHTML={{ __html: isStopped ? '&#9632; 已停止' : isPaused ? '&#9208; 暂停' : '● 运行中' }}
          />
      ...
      <div>
        ...
        <div className="bg-card border border-border/50 rounded-xl overflow-hidden shadow-sm">
          <table className="w-full">
            ...
            <tbody>
              {[
                { time: '03-22 09:42', dir: '买入', price: '83,940', pnl: isStopped ? '已平仓' : '持仓中', isBuy: true, isPending: !isStopped, isUp: true },
                { time: '03-19 14:10', dir: '卖出', price: '85,140', pnl: '+¥342', isBuy: false, isPending: false, isUp: true },
                { time: '03-15 08:20', dir: '买入', price: '80,820', pnl: '—', isBuy: true, isPending: true, isUp: true },
